...
angular.module('hackstack.feedhenry', [
  ...
  .factory('mockFH', function (hackStack, $window) {
    ...
    function mockCloudFn(options, successCallback, failureCallback) {
      var path, method, hs, backendResponse;

      path = options.path;
      method = options.method;

      // TODO look at options.method
      hs = mockEndpoints[path];

      if (hs === undefined) {
        throw new Error('This endpoint has not been defined: '.concat(path));
      }

      /* jshint -W116 */
      // catches undefined, false, null
      // TODO change how this works. fh.forceError(404).get();
      ...
      backendResponse.then(
        ...
        function (result) {
          successCallback(null, wrapBackendResponse(result));
        });
    }
