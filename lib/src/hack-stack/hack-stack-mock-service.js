...
angular.module('hackstack.mock', [
  ...
  .factory('hackStack', function ($http, $q, hackStackUtils) {
    ...
    function createMock(mockData, options) {
      ...
      function processQuery(reqObj) {
        var keys = R.keys(reqObj);
        // create new objects that only include the keys in the query
        var comparisonData = R.map(R.pick(keys))(mockData);
        var foundIndex = R.findIndex(R.eqDeep(reqObj))(comparisonData);
       ...
       *
       * An object matches the query object
       *
       * @param reqObj
       * @returns {*}
       */
      function query(reqObj) {
        return waitForTime().then(function () {
          return $q.when(processQuery(reqObj));
        });
      }
