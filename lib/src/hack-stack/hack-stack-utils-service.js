...
angular.module('hackstack.common', [])
  ...
  .factory('hackStackUtils', function ($timeout, $window) {
    ...
    function evaluateTriggers(data, method) {
      ...
      }, R.filter(R.propEq('method', method.toLowerCase()), errorTriggers));
      
      return error;
    }
    
    /**
     * External function that hackstack uses to determine if an error is to
     * be returned to the user.  It first checks error triggers and if none
     * are fired, then it will possibly return a random error.
     * 
    ...
    function addErrorTrigger(errorFn, errorCode, method) {
      ...
      return function removeTrigger() {
        ...
        var removeIndex = R.findIndex(R.propEq('id', myerrorTriggerId))(errorTriggers);
